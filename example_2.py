...
# choose flap type
# 0 for main flap only
# 1 for main flap with auxilary
# 2 for main flap with plain flap
flap_type = 2

# choose positions
# main flap position
mfpi = 2        # Takes integers from 0 to 5 for ss_flap, 0 to 2 for ds_flap, 0 to 4 for sp_flap
# auxilary flap position
afpi = 3        # Takes integers from 0 to 5 for mfpi 0-1, 0 to 4 for mfpi 2
# plain flap position
pfpi = 3        # Takes integers from 0 to 6 for mfpi 0-2, 0 to 5 for mfpi 3, 0 to 4 for mfpi 4
...
if flap_type == 0:
        ...
        mfle = [74.34, -1.29]
        afte = [82.7, 2.64]
        path = np.array([[0,8.36,3.91], [10,5.41,3.63], [20,3.83,3.45], [30,2.63,3.37], [40,1.35,2.43], [50,0.5,1.63]])
    ...
    theta = np.deg2rad([0, 10, 20, 30, 40, 50, 60])
    main_flap = main_flap.te_flap(90, 0, theta[pfpi])
    main_flap.weld([0,1])
    # flap deflection
    # path of main flap
    mfle = [74.34, -1.29]
    afte = [82.7, 2.64]
    path = np.array([[0,8.36,3.91], [10,5.41,3.63], [20,3.83,3.45], [30,2.63,3.37], [40,1.35,2.43]])
    path[:,1:3] = np.tile(afte, (len(path),1)) - path[:,1:3]
    # translate and rotate flap
    main_flap.rotate(mfle, -path[mfpi,0]*np.pi/180)
    main_flap.translate(path[mfpi,1:3]-mfle)
...
md.inflation_layer(60, 6.5, 150)
md.tangent_spacing(13, 0.84, 0.2, 0.5, [10, 50], 50, [70,26])
md.normal_spacing(0.00002, 2, 1.16)
md.wake_region(70, [-13*np.pi/180, 13*np.pi/180], 68, 16)
t1 = time()
